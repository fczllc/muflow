...
- [x] 组件配置能力增强
  - [x] 支持通过props传递配置参数
  - [x] 集成依赖注入机制实现子组件配置
  - [x] canvasTools参数支持配置画布工具按钮的可见性
- [x] 暴露API方法
  - [x] 暴露关键方法供外部调用(exportFlowData, getDataUrl等)
  - [x] 提供类型定义支持TypeScript类型检查
  - [x] 添加ref引用方式调用组件方法的示例
- [x] 导出图像能力增强
  - [x] 支持导出为JPG/PNG格式
  - [x] 优化导出图像质量和精度
  - [x] 实现基于有效内容区域的智能裁剪
...
1. 鱼骨图模板功能
   - 增加了一键创建标准鱼骨图模板的功能
   - 模板包含主骨干和侧骨结构，便于快速创建因果分析图
   - 提供确认对话框，防止误操作清空当前画布
   - 优化了鱼骨图节点的样式和位置关系
   - 与图标库保持一致的UI设计，提升用户体验
...
1. 基本操作
   - 点击左侧栏的鱼骨图按钮快速创建标准鱼骨图框架
   - 系统会弹出确认对话框，提醒用户当前画布内容将被清空
   - 确认后，系统会自动创建一个包含主干和侧骨的标准鱼骨图结构

2. 鱼骨图定制
   - 主干线：水平贯穿画布中央的主线条，可调整长度和位置
   - 侧骨线：从主干向上下延伸的线条，默认45度角排列
   - 每条线都可单独选中，调整其位置、长度、样式和角度
   - 可添加文字标签说明每个骨节点代表的内容

3. 使用场景
   - 因果分析：识别问题的根本原因和相关因素
   - 流程优化：分析流程中的各个环节和影响因素
   - 项目规划：梳理项目各阶段的关键点和依赖关系
   - 风险评估：识别风险来源和潜在影响

4. 最佳实践
   - 主干线通常表示问题或结果
   - 侧骨线表示导致该问题的主要因素类别
   - 可进一步添加次级骨线，表示更详细的子因素
   - 使用不同颜色区分不同类别的因素
   - 使用文本标签标注每个因素的具体内容
